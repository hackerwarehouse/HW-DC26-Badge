...
qMenuSystem::qMenuSystem(SSD_13XX *disp)
{
...
if (CurrentMenu == (const char **)"SSID List")
	ShowMenuSSID();
else
	ShowMenu();
...
void qMenuSystem::ShowMenu()
{
  ...
  strcpy_P(tempBuffer, (char*) pgm_read_dword(&(CurrentMenu[0]))); 
  ...
  for (int i=0;i<p;i++)
  {
    strcpy_P(tempBuffer, (char*)pgm_read_dword(&(CurrentMenu[i+_firstVisible])));
...
void qMenuSystem::InitMenuSSID(const char ** page, int itemCount, int selectedIndex)
{
  qmd.Begin();
  CurrentMenu=page;
  _selectedIndex=selectedIndex;
